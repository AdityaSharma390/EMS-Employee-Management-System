...
export function EmployeesClient() {
  ...
  const fetchEmployees = async () => {
    ...
    try {
      ...
      const res = await clientFetch(`/api/employees?${query}`);
      if (res.ok) {
  ...
  const fetchDepartments = async () => {
    try {
      const res = await clientFetch("/api/departments");
      if (res.ok) {
        const data = await res.json();
  ...
  const onAddSubmit = async (data: EmployeeInput) => {
    setFormError(null);
    try {
      const res = await clientFetch("/api/employees", {
        method: "POST",
  ...
  const onEditSubmit = async (data: EmployeeInput) => {
    ...
    setFormError(null);
    try {
      const res = await clientFetch(`/api/employees/${editingEmployee.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(data),
      });
  ...
  const handleDelete = async (id: string, name: string) => {
    if (confirm(`Are you sure you want to delete ${name}? This will remove all their records.`)) {
      try {
        const res = await clientFetch(`/api/employees/${id}`, {
          method: "DELETE",
        });
        if (res.ok) {
          fetchEmployees();
